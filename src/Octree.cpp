...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	...
	subDivideBox8(node.box, childBoxes); // 1) subdivide box in node into 8 equal side boxes - see helper function subDivideBox8().
	for(Box childBox : childBoxes) { // 2) For each child box
		vector<int> facesRtn;
		int numFace = getMeshPointsInBox(mesh, node.points, childBox, facesRtn); // sort point data into each box  (see helper function getMeshFacesInBox()). I used getMeshPointsInBox() instead, since using the latter doesn't provide full coverage of the map for some reason.
		if(numFace >= 1) { //if a child box contains at list 1 point add child to tree
			TreeNode childNode;
			childNode.box = childBox;
			childNode.points = facesRtn;
			node.children.push_back(childNode);
			
			TreeNode &ref = node.children.back();
			if(numFace > 1) subdivide(mesh, ref, numLevels, level + 1); // if child is not a leaf node (contains more than 1 point) recursively call subdivide(child). We use a reference because if we don't, it'll create a new copy of itself.
		}
	}
}
...
//
// Start from root node and recursively traverse through entire octree.
// Check if the box of that node interesects with the ray.
// 		If it does, we check the children nodes
// No more children to check, means that the ray intersects at that particular box.
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
	bool intersects = false;
	float t0 = 0.0f;
	float t1 = FLT_MAX;
	if(node.children.size() == 0 && node.box.intersect(ray, t0, t1)) {
		nodeRtn = node;
		intersects = true;
	}
	if(node.children.size() > 0 && node.box.intersect(ray, t0, t1)) {
		for(const TreeNode & childNode : node.children)
			intersects |= intersect(ray, childNode, nodeRtn);
	}
	return intersects;
}
// Check if node box intersects with given box.
// If it does check the corresponding subtrees.
bool Octree::intersect(const Box &box, TreeNode & node, vector<Box> & boxListRtn) {
	bool intersects = false;
	if(node.children.size() == 0 && node.box.overlap(box)) {
		boxListRtn.push_back(node.box);
		intersects = true;
	}
	if(node.children.size() > 0 && node.box.overlap(box)) {
		for(TreeNode & childNode : node.children)
			intersects |= intersect(box, childNode, boxListRtn);
	}
	return intersects;
}

void Octree::draw(TreeNode & node, int numLevels, int level, vector<ofColor> &colors) {
	// We want to draw from the root all the way to the leaf nodes.
	if(level >= numLevels) return;
	if(level < colors.size()) {
		ofSetColor(colors[level]);
	} else {
		ofSetColor(ofColor::white); // fallback
	}
	drawBox(node.box);
//	if (level == numLevels) {          // draw exactly at target depth
//		drawBox(node.box);
//		return;
//	}
	if(node.children.size() == 0) return;
	for(TreeNode & childNode : node.children) {
		draw(childNode, numLevels, level + 1, colors);
	}
}
